...
- **Granting super_admin:** Bootstrap the first platform admin from a shell with `pnpm grant-super-admin <email>` (the user must already have signed up via Clerk and completed onboarding so a tenant_membership row exists). Pass `--revoke` to demote back to `tenant_admin`. Once at least one super_admin exists, additional super_admins can be granted/revoked from the UI: Super Admin → Tenant detail → Members → "Make super admin" / "Revoke super admin" button. The PATCH and CLI both enforce a platform-wide last-super_admin guard (`code: "LAST_SUPER_ADMIN"`) so the platform can never be locked out. To wipe all tenant data in dev, run `bash scripts/reset-tenant-data.sh` (deletes from `tenants` cascading to all tenant-scoped tables, then truncates `audit_logs`, `onboarding_sessions`, `users`).
