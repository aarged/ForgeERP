...
- **Role-Based Access Control (RBAC):** Defined roles (`global_admin`, `tenant_admin`, `purchaser`, `warehouse`, `approver`, `accountant`, `viewer`) govern access.
- **Granting global_admin:** Bootstrap the first platform admin from a shell with `pnpm grant-global-admin <email>` (the user must already have signed up via Clerk and completed onboarding so a tenant_membership row exists). Pass `--revoke` to demote back to `tenant_admin`. Once at least one global_admin exists, additional global_admins can be granted/revoked from the UI: Global Admin → Tenant detail → Members → "Make global admin" / "Revoke global admin" button. The PATCH and CLI both enforce a platform-wide last-global_admin guard (`code: "LAST_GLOBAL_ADMIN"`) so the platform can never be locked out. To wipe all tenant data in dev, run `bash scripts/reset-tenant-data.sh` (deletes from `tenants` cascading to all tenant-scoped tables, then truncates `audit_logs`, `onboarding_sessions`, `users`).
- **Audit Logging:** A comprehensive `audit_logs` table records all system actions. Global admins can browse audit history via the "Audit Logs" tab on `/global-admin`, with client-side filtering by tenant and action and 50-per-page pagination.
