...
export const inventoryMovementsTable = pgTable("inventory_movements", {
  ...
  warehouseId: integer("warehouse_id")
    ...
    .references(() => warehousesTable.id, { onDelete: "cascade" }),
  locationId: integer("location_id").references(() => warehouseLocationsTable.id),
  movementType: text("movement_type").notNull(), // receipt | return | adjustment | transfer | issue
  quantity: numeric("quantity", { precision: 18, scale: 4 }).notNull(), // positive = in, negative = out
  unitCost: numeric("unit_cost", { precision: 18, scale: 4 }),
  refType: text("ref_type"), // po_receipt | po_return
